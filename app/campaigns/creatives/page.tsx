...
import { ImageIcon, Sparkles, Loader2, Search, RotateCw } from "lucide-react";
  ...
  const creativeList = useMemo(() => {
    ...
    safeArray(hierarchy?.ads).forEach(ad => {
      if (map[ad.id]) {
        ...
        if (c && !safeHD[ad.id]) {
          map[ad.id]._thumb = c.thumbnail_url || c.image_url;
        }
      }
    });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10">
            ...
            <span className="text-[10px] font-bold text-muted uppercase tracking-widest">{creativeList.length} Criativos</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {creativeList.map((creative: any) => {
          ...
          return (
            <ErrorBoundary key={id} name={`creative-${id}`}>
              <div className="glass overflow-hidden group hover:scale-[1.02] transition-all">
                <div className="aspect-video bg-white/5 relative overflow-hidden">
                  {thumb
                    ...
                    : <div className="w-full h-full flex items-center justify-center"><ImageIcon className="w-8 h-8 text-muted/20" /></div>}
                ...
                <div className="p-3 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { label: "Gasto", value: formatCurrency(creative._spend), color: "text-white" },
                      { label: "Conversas", value: formatNumber(creative._convs), color: "text-warning" },
                      { label: "Impressões", value: formatNumber(creative._impressions), color: "text-white" },
                      { label: "CPL", value: cpl > 0 ? formatCurrency(cpl) : "—", color: "text-white" },
                    ].map(m => (
                      <div key={m.label} className="bg-white/[0.03] rounded-lg p-2">
                        <span className="text-[9px] font-bold text-muted uppercase block">{m.label}</span>
                        <span className={cn("text-xs font-bold mono", m.color)}>{m.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
